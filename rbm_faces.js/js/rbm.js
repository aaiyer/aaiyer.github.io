...
function RBM(n_visibles,
    ...
    var v_scaling = Math.sqrt(visibles_canvas.width()*visibles_canvas.height()/n_visibles);
    var h_scaling = Math.sqrt(hiddens_canvas.width()*hiddens_canvas.height()/n_hiddens);
...
RBM.prototype.sample_v = function(h, use_mean) {
    ...
    for (var i = 0; i < this.n_visibles; i++) {
        ...
        if (use_mean === undefined || use_mean == false) {
            ...
            this.visibles_context.fillStyle = "rgb(" + intensity + ", " + intensity + ", " + intensity + ")";
            this.visibles_context.fillRect(row, column, 1, 1);
